#!/usr/bin/env python3
"""Re-ingest PANDiT data with fixed adapter."""
import asyncio
import sys
import psycopg2
...
async def reingest_pandit():
    ...
    print("=== RE-INGESTING PANDiT DATA ===")
    print()
    
    # Get all PANDiT entities
    result = await adapter.discover(limit=200)
    items = result['items']
    print(f"Found {len(items)} PANDiT entities")
    print()
    ...
    cur.execute('''
        SELECT COUNT(*)
        FROM assertions
        WHERE LENGTH(literal) <= 1
    ''')
    single_char = cur.fetchone()[0]
    print(f"Single-character assertions: {single_char}")
